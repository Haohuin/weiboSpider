...
class Weibo(object):
    ...
    def user_to_mysql(self):
        ...
        # 创建'weibo_author'表
        create_table = """
                CREATE TABLE IF NOT EXISTS weibo_author (
                ...
                author_topic varchar(20),
                PRIMARY KEY (id)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"""
        self.mysql_create_table(mysql_config, create_table)
        self.mysql_insert(mysql_config, 'weibo_author', [self.user])
        logger.info(u'%s信息写入MySQL数据库完毕', self.user['author_name'])
    ...
    def get_user_info(self):
        ...
        if js['ok']:
            ...
            user_info['author_name'] = info.get('screen_name', '')
            user_info['gender'] = info.get('gender', '')
            params = {
                'containerid':
                    '230283' + str(self.user_config['user_id']) + '_-_INFO'
            }
            # u'生日', , u'小学', u'初中', u'高中', u'大学', u'阳光信用', u'公司', u'注册时间'
            zh_list = [
                u'所在地'

            ]
            # 'birthday','education', 'education', 'education', 'education','company',, 'sunshine', 'registration_time'
            en_list = [
                'location'
            ]
            for i in en_list:
                    ...
                    for card in card_list:
                        if card.get('item_name') in zh_list:
                            user_info[en_list[zh_list.index(
                                ...
                                'item_content', '')
            user_info['weibo_count'] = self.string_to_int(
            ...
            logger.info(u"被ban了,查看一下id是否正确")
            # sys.exit()
    ...
    def handle_download(self, file_type, file_dir, urls, w):
        """处理下载相关操作"""
        file_prefix = w['create_time'][:11].replace('-', '') + '_' + str(
            w['id'])
        if file_type == 'img':
            if ',' in urls:
                url_list = urls.split(',')
    ...
    def standardize_date(self, create_time):
        """标准化微博发布时间"""
        if u'刚刚' in create_time:
            create_time = datetime.now().strftime('%Y-%m-%d')
        elif u'分钟' in create_time:
            minute = create_time[:create_time.find(u'分钟')]
            minute = timedelta(minutes=int(minute))
            create_time = (datetime.now() - minute).strftime('%Y-%m-%d')
        elif u'小时' in create_time:
            hour = create_time[:create_time.find(u'小时')]
            hour = timedelta(hours=int(hour))
            create_time = (datetime.now() - hour).strftime('%Y-%m-%d')
        elif u'昨天' in create_time:
            day = timedelta(days=1)
            create_time = (datetime.now() - day).strftime('%Y-%m-%d')
        else:
            create_time = create_time.replace('+0800 ', '')
            temp = datetime.strptime(create_time, '%c')
            create_time = datetime.strftime(temp, '%Y-%m-%d')
        return create_time
    ...
    def parse_weibo(self, weibo_info):
        weibo = OrderedDict()
        if weibo_info['user']:
            weibo['author_id'] = weibo_info['user']['id']
            weibo['author_name'] = weibo_info['user']['screen_name']
        else:
            weibo['author_id'] = ''
            weibo['author_name'] = ''
        ...
        if self.remove_html_tag:
            weibo['content'] = selector.xpath('string(.)')
        else:
            weibo['content'] = text_body
        weibo['article_url'] = "https://m.weibo.cn/detail/" + str(weibo['id'])
        # weibo['pics'] = self.get_pics(weibo_info)
        # weibo['video_url'] = self.get_video_url(weibo_info)
        weibo['location'] = self.get_location(selector)
        weibo['create_time'] = weibo_info['created_at']
        # weibo['source'] = weibo_info['source']
        weibo['like_count'] = self.string_to_int(
            weibo_info.get('attitudes_count', 0))
        weibo['comment_count'] = self.string_to_int(
            weibo_info.get('comments_count', 0))
        weibo['transmit_count'] = self.string_to_int(
    ...
    def print_user_info(self):
        ...
        logger.info(u'用户昵称：%s', self.user['author_name'])
        ...
        logger.info(u'微博数：%d', self.user['weibo_count'])
    ...
    def print_one_weibo(self, weibo):
        ...
        try:
            ...
            logger.info(u'微博正文：%s', weibo['content'])
            ...
            logger.info(u'点赞数：%d', weibo['like_count'])
            # logger.info(u'评论数：%d', weibo['comment_count'])
            logger.info(u'转发数：%d', weibo['transmit_count'])
        ...
        try:
            ...
            if retweeted_status and retweeted_status.get('id'):  # 转发
                ...
                if is_long_retweet:
                    ...
                    retweet = self.parse_weibo(retweeted_status)
                retweet['create_time'] = self.standardize_date(
                ...
                if is_long:
                    ...
                    weibo = self.parse_weibo(weibo_info)
            weibo['create_time'] = self.standardize_date(
    ...
    def get_one_page(self, page):
        ...
        try:
            ...
            logger.info(u'{}已获取{}({})的第{}页微博{}'.format(
                '-' * 30, self.user['author_name'], self.user['id'], page,
                '-' * 30))
        except Exception as e:
            logger.exception(e)
    ...
    def get_page_count(self):
        """获取微博页数"""
        try:
            weibo_count = self.user['weibo_count']
